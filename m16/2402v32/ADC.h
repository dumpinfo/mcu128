...
unsigned char led_buff[16]={0x53,0x74,0x50,0x3a,0,0,0,0x20,0x50,0x72,0x65,0x73,0x3a,0,0,0};
unsigned char key_buff[16]={0x53,0x74,0x54,0x3a,0,0,0,0x20,0x54,0x69,0x6d,0x65,0x3a,0,0,0};
...
unsigned int ADCtoBCD(unsigned int temp,unsigned int temp1,unsigned int choose)
 {
  ...
  unsigned char i;
  if(choose==1)
   { temp=tt=((52.69*(unsigned int)(((unsigned long int)((unsigned long int)temp*Vref))/0x3ff))-208)*0.0075;
  for(i=0;i<2;i++)
  {
   led_buff[14-i]=seg_table[temp%10];
   temp=temp/10;
   led_buff[5-i]=seg_table[temp1%10];
   temp1=temp1/10;
   led_buff[6]=led_buff[15]=0X20;
   }
   }
  else
   for(i=0;i<2;i++)
  {
   ...
   temp=temp/10;
   key_buff[5-i]=seg_table[temp1%10];
   temp1=temp1/10;
   key_buff[6]=key_buff[15]=0X20;
   }
   return tt;
 }