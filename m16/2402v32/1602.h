...
#define lcd_RS (1<<0) //PORTD^0
#define lcd_RW (1<<1) //PORTD^1
#define lcd_EN (1<<2) //PORTD^2
...
void display_a_string(unsigned char col,unsigned char *ptr)//参数col指定行,*ptr指字符串数组的首指针
{
 ...
 col_tem=col*24;//若col为1(即在LCD第二行显示字符串),先把col左移4位,使显示字符的首位置改到第二行首位,即位置16
 for(i=0;i<24;i++)
